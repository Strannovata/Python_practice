...
def game_command(update: Update, context: CallbackContext):
    ...
    update.message.reply_text(f'Давайте сыграем в игру с конфетами! Игроки по очереди берут не больше оговоренного количества конфет. Победит тот, кому достанутся последние конфеты! Для начала установим правила:')
    update.message.reply_text(f'Сколько у вас всего конфет?')
    msg = update.message.text
    colsweet = int(msg)
    update.message.reply_text(f'Какое максимальное количество конфет будете брать за раз?')
    msg = update.message.text
    ingive = int(msg)
    update.message.reply_text(f'Ок, всего {colsweet} конфет, и берем не более {ingive}')
    # player = 1
    # plgive = 0
